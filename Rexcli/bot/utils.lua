...
assert (tdbot_function ({
  ...
  if not lang then
    return tdbot.sendText(msg.chat_id, msg.id, "*₪ All group admins has been ➲ promoted and group creator is now ➲ group owner😎*", 0, 1, nil, 0, 'md', 0, nil)
else
    return tdbot.sendText(msg.chat_id, msg.id, "*₪ تمام ادمین های گروه به مقام مدیر منتصب شدند و سازنده گروه به مقام مالک گروه منتصب شد😎*", 0, 1, nil, 0, 'md', 0, nil)
     end
 end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(msg.chat_id)]['filterlist']) == nil then --fix way
    ...
    return "↜ لیست کلمات فیلتر شده خالی است_"
     end
  end
  if not data[tostring(msg.chat_id)]['filterlist'] then
    data[tostring(msg.chat_id)]['filterlist'] = {}
