...
--Edited Warn.lua By @Rex_Developer #Rex™
...
redis:hdel(hashwarn, BD.id, '0')
    if not lang then
    return tdbot.sendMessage(TM.chat_id, "", 0, "⇋ User "..user_name.." `"..BD.id.."` ➲ *kicked* because max *warning*⚠️\nNumber of warn : "..warnhash.."/"..max_warn.."", 0, "md")
    else
...
redis:hdel(hashwarn, BD.id, '0')
   if not lang then
    return tdbot.sendMessage(TM.chat_id, "", 0, "⇋ All warn of "..user_name.." `"..BD.id.."` ➲ *cleaned*", 0, "md")
   else
...
local Clang = redis:get(Chash)
    ...
    redis:del(hash)
    if not lang then
     return "⇋ All *warn* of *users* in this *group* ➲ *cleaned*"
   else
